#!/usr/bin/env python3
"""
Convertit tous les fichiers .md du dossier docs en format Linux (LF) + UTF-8
Usage: python text2linux.py
"""
...
def convert_to_lf(file_path):
    """Convertit un fichier en format Linux (LF) avec encodage UTF-8"""
    try:
        ...
        if content is None:
            ...
            with open(file_path, 'rb') as f:
                raw_content = f.read()
            # Remplacer CRLF par LF en binaire
            raw_content = raw_content.replace(b'\r\n', b'\n')
            with open(file_path, 'wb') as f:
                f.write(raw_content)
            return True

        # Écrire en UTF-8 avec LF
...
def main():
    ...
    print(f"Conversion des fichiers .md en format Linux (LF)...")
