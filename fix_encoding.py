#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Script pour corriger les problèmes d'encodage UTF-8 dans les fichiers du projet.
Usage: python3 fix_encoding.py [chemin_du_fichier_ou_dossier]
"""

import os
import sys
import chardet
from pathlib import Path

# Extensions de fichiers à traiter
EXTENSIONS = {'.php', '.blade.php', '.js', '.css', '.md', '.txt', '.json', '.env', '.yml', '.yaml'}

# Fichiers/dossiers à ignorer
IGNORE_PATTERNS = {
    'vendor', 'node_modules', '.git', 'storage', 'bootstrap/cache',
    '.vscode', '.idea', 'public/build', 'composer.lock', 'package-lock.json'
}

def should_process(file_path):
    """Vérifie si le fichier doit être traité"""
    # Ignorer les patterns
    for pattern in IGNORE_PATTERNS:
        if pattern in str(file_path):
            return False

    # Vérifier l'extension
    if file_path.suffix in EXTENSIONS:
        return True

    # Cas spécial pour .blade.php
    if file_path.name.endswith('.blade.php'):
        return True

    return False

def detect_encoding(file_path):
    """Détecte l'encodage d'un fichier"""
    try:
        with open(file_path, 'rb') as f:
            raw_data = f.read()
            result = chardet.detect(raw_data)
            return result['encoding'], result['confidence']
    except Exception as e:
        print(f"❌ Erreur lors de la détection de {file_path}: {e}")
        return None, 0

def convert_to_utf8(file_path, dry_run=False):
    """Convertit un fichier en UTF-8"""
    encoding, confidence = detect_encoding(file_path)

    if encoding is None:
        return False

    # Si déjà en UTF-8 avec bonne confiance
    if encoding.lower() in ['utf-8', 'ascii'] and confidence > 0.7:
        print(f"✓ {file_path} - Déjà en UTF-8")
        return True

    print(f"🔄 {file_path}")
    print(f"   Encodage détecté: {encoding} (confiance: {confidence:.2%})")

    if dry_run:
        print(f"   [DRY-RUN] Serait converti en UTF-8")
        return True

    try:
        # Lire avec l'encodage détecté
        with open(file_path, 'r', encoding=encoding, errors='replace') as f:
            content = f.read()

        # Écrire en UTF-8
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(content)

        print(f"   ✅ Converti en UTF-8")
        return True

    except Exception as e:
        print(f"   ❌ Erreur: {e}")
        return False

def process_path(path, dry_run=False):
    """Traite un fichier ou un dossier"""
    path = Path(path)

    if path.is_file():
        if should_process(path):
            convert_to_utf8(path, dry_run)
    elif path.is_dir():
        print(f"\n📁 Traitement du dossier: {path}\n")
        files_processed = 0
        files_converted = 0

        for file_path in path.rglob('*'):
            if file_path.is_file() and should_process(file_path):
                files_processed += 1
                if convert_to_utf8(file_path, dry_run):
                    files_converted += 1

        print(f"\n📊 Résumé:")
        print(f"   Fichiers traités: {files_processed}")
        print(f"   Fichiers convertis: {files_converted}")
    else:
        print(f"❌ Chemin invalide: {path}")

def main():
    print("=" * 60)
    print("🔧 Correcteur d'encodage UTF-8")
    print("=" * 60)

    # Vérifier si chardet est installé
    try:
        import chardet
    except ImportError:
        print("\n❌ Le module 'chardet' n'est pas installé.")
        print("   Installez-le avec: pip3 install chardet")
        sys.exit(1)

    # Arguments
    dry_run = '--dry-run' in sys.argv or '-n' in sys.argv

    if dry_run:
        print("\n⚠️  MODE DRY-RUN - Aucune modification ne sera effectuée\n")

    # Chemin à traiter
    if len(sys.argv) > 1 and not sys.argv[1].startswith('-'):
        target = sys.argv[1]
    else:
        target = '.'

    process_path(target, dry_run)

    print("\n" + "=" * 60)
    print("✅ Traitement terminé")
    print("=" * 60)

if __name__ == '__main__':
    main()
